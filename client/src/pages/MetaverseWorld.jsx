  ...
  useEffect(() => {
    if (!loadingPage && space && canvasRef.current) {
      ...
      const initGame = (app) => {
        ...
        // renderDebugColliders(gameState.colliders, gameState.camera);
...
const createCat = (x, y,f1= true, camera, colliders) => {
  ...
  const createCatParts = () => {
    ...
    if(f1===true)
    body.beginFill(0xf4d03f);//B2BEB5
    else
    body.beginFill(0xB2BEB5);//B2BEB5
    ...
    if(f1===true)
    stripes.beginFill(0xe67e22);//7393B3
    else
    stripes.beginFill(0x7393B3);//7393B3
    ...
    if(f1===true)
    tail.beginFill(0xf4d03f);
    else
    tail.beginFill(0xB2BEB5);
    ...
    if(f1===true)
    tailStripes.beginFill(0xe67e22);
    else
    tailStripes.beginFill(0x7393B3);
    ...
    if(f1===true)
    head.beginFill(0xf4d03f);
    else
    head.beginFill(0xB2BEB5);
    ...
    if(f1===true)
    headStripes.beginFill(0xe67e22);
    else
    headStripes.beginFill(0x7393B3);
    ...
    if(f1===true)
    leftEar.beginFill(0xf4d03f);
    else
    leftEar.beginFill(0xB2BEB5);
    leftEar.drawPolygon([-6, -10, -2, -18, 2, -12]);
    leftEar.endFill();
    
    const rightEar = new Graphics();
    if(f1===true)
    rightEar.beginFill(0xf4d03f);
    else
    rightEar.beginFill(0xB2BEB5);
    ...
    if(f1===true)
    leftInnerEar.beginFill(0xf39c12);
    else
    leftInnerEar.beginFill(0xE2DFD2);
    leftInnerEar.drawPolygon([-4, -11, -2, -15, 0, -12]);
    leftInnerEar.endFill();
    
    const rightInnerEar = new Graphics();
    if(f1===true)
    rightInnerEar.beginFill(0xf39c12);
    else
    rightInnerEar.beginFill(0xE2DFD2);
...
rightEye.stroke();
    ...
    return {
      shadow, body, stripes, tail, tailStripes, head, headStripes,
      leftEar, rightEar, leftInnerEar, rightInnerEar,
      leftEye, rightEye, nose, mouth, whiskers,
...
const createWalls = (roomWidth, roomHeight, row, col, grid, colliders, offsetX, offsetY) => {
  const walls = [];
  const thick = 32;
  const thin = 12;
  const gapSize = 80;
  ...
  if (hasRoomAbove) {
    ...
    colliders.push({ x: offsetX + 0, y: offsetY + 0, width: visualGapStart, height: topThickness });
    colliders.push({ x: offsetX + visualGapEnd, y: offsetY + 0, width: roomWidth - visualGapEnd, height: topThickness });

  } else {
    // No door - solid wall
    topWall.rect(0, 0, roomWidth, topThickness);
    colliders.push({ x: offsetX + 0, y: offsetY + 0, width: roomWidth, height: topThickness });
  }
  ...
  if (hasRoomBelow) {
    ...
    bottomWall.rect(gapX + gapSize, bottomY, roomWidth - (gapX + gapSize), bottomThickness);
    
    // Create collision boxes with IDENTICAL calculations
    colliders.push({ x: offsetX + 0, y: offsetY + bottomY, width: gapX, height: bottomThickness });
colliders.push({ x: offsetX + gapX + gapSize, y: offsetY + bottomY, width: roomWidth - (gapX + gapSize), height: bottomThickness });

  } else {
    // No door - solid wall
    bottomWall.rect(0, bottomY, roomWidth, bottomThickness);
    colliders.push({ x: offsetX + 0, y: offsetY + bottomY, width: roomWidth, height: bottomThickness });
  }
  ...
  if (hasRoomLeft) {
    ...
    leftWall.rect(0, visualGapEnd, leftThickness, roomHeight - visualGapEnd);

    // Create collision boxes that EXACTLY match visual walls
colliders.push({ x: offsetX + 0, y: offsetY + 0, width: leftThickness, height: visualGapStart });
colliders.push({ x: offsetX + 0, y: offsetY + visualGapEnd, width: leftThickness, height: roomHeight - visualGapEnd });

  } else {
    // No door - solid wall
    leftWall.rect(0, 0, leftThickness, roomHeight);
    colliders.push({ x: offsetX + 0, y: offsetY + 0, width: leftThickness, height: roomHeight });
  }
  ...
  if (hasRoomRight) {
    ...
    rightWall.rect(rightX, visualGapEnd, rightThickness, roomHeight - visualGapEnd);

    // Create collision boxes that EXACTLY match visual walls
colliders.push({ x: offsetX + rightX, y: offsetY + 0, width: rightThickness, height: visualGapStart });
colliders.push({ x: offsetX + rightX, y: offsetY + visualGapEnd, width: rightThickness, height: roomHeight - visualGapEnd });

  } else {
    // No door - solid wall
    rightWall.rect(rightX, 0, rightThickness, roomHeight);
    colliders.push({ x: offsetX+rightX, y: offsetY + 0, width: rightThickness, height: roomHeight });
  }
...
const createCubicle = (x, y, width = 120, height = 100) => {
  ...
  if (colliders) {
    ...
    // 🧱 Wall Colliders
    colliders.push({ x: offsetX, y: offsetY, width: width, height: 8, label: "cubicle-back" });
...
const createReceptionCounter = (x, y, camera, colliders) => {
  ...
  window.interactables.push({
    label: "elevator",
    bounds,
    message: "An unmanned reception desk. The screen is active, but there's nothing to do here",
    bubble: null
  });
  ...
  const createSingleCounter = (counterX, counterY, counterIndex) => {
    ...
    const createLaptopAnimation = () => {
      ...
      if (counterIndex === 0) {
        ...
        screenContent.rect(counterX - 8, counterY - 24 + emailOffset, 16, 0.8);
        screenContent.fill(0xffffff);
        screenContent.rect(counterX - 8, counterY - 22 + emailOffset, 14, 0.8);
        screenContent.fill(0xecf0f1);
        screenContent.rect(counterX - 8, counterY - 20 + emailOffset, 15, 0.8);
        screenContent.fill(0xffffff);
        ...
        for (let i = 0; i < 6; i++) {
          screenContent.rect(counterX - 9, counterY - 26 + i * 1.5, 18, 0.3);
          screenContent.fill(0x7f8c8d);
        }
        ...
        screenContent.rect(counterX - 9 + cellX * 4, counterY - 26 + cellY * 1.5, 4, 1.5);
        screenContent.fill(0x3498db);
        ...
        screenContent.rect(counterX - 8, counterY - 25 + codeScroll, 10, 0.8);
        screenContent.fill(0xe74c3c);
        screenContent.rect(counterX - 8, counterY - 23 + codeScroll, 14, 0.8);
        screenContent.fill(0x2ecc71);
        screenContent.rect(counterX - 8, counterY - 21 + codeScroll, 8, 0.8);
        screenContent.fill(0xf39c12);
        screenContent.rect(counterX - 8, counterY - 19 + codeScroll, 12, 0.8);
    ...
    const createSteamAnimation = () => {
      ...
      for (let i = 0; i < 3; i++) {
        const wispX = counterX + 22 + i * 1.5 + Math.sin(steamTime * 2 + i) * 1.5;
        const wispY = counterY - 10 - (steamTime * 8 + i * 4) % 12;
        const opacity = Math.max(0, 1 - ((steamTime * 8 + i * 4) % 12) / 12);
...
for (let i = 0; i < n; i++) {
  ...
  if (i === 0) {
    ...
    if (camera && colliders) {
      addWithCollider(camera, leftWall, {
        x: counterX - 50,
        y: y - 38,
        width: 6,
        height: 66,
  ...
  if (i === n-1) {
    ...
    if (camera && colliders) {
      addWithCollider(camera, rightWall, {
        x: counterX + 45,
        y: y - 38,
        width: 6,
        height: 66,
  ...
  if (i !== 0) {
    ...
    if (camera && colliders) {
      addWithCollider(camera, sideWall, {
        x: counterX - 50,
        y: y - 38,
        width: 6,
        height: 66,
  ...
  if (camera && colliders) {
    addWithCollider(camera, counter, {
      x: counterX - 40,
      y: y - 25,
      width: 80,
      height: 50,
    ...
    addWithCollider(camera, backWall, {
      x: counterX - 45,
      y: y - 36,
      width: 90,
      height: 6,
...
const bounds = {
  ...
  window.interactables.push({
    label: "lobby-seating",
    bounds,
    message: "This sofa seems inviting, but it's part of the office decor",
    bubble: null
  });
...
const createVerticalLeftSofa = (x, y, camera, colliders) => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    sofa.drawRoundedRect(cushionX + 1, cushionY + cushionHeight - 6, 33, 3, 2);
    sofa.endFill();
 ...
 const createWindows = (roomWidth, roomHeight) => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // Top-right corner
    window.rect(x + windowWidth + 2 - cornerSize, 3, cornerSize, cornerSize);
    window.fill(0x1a252f);
    // Bottom-left corner
    window.rect(x - 2, 5 + windowHeight + 2 - cornerSize, cornerSize, cornerSize);
    window.fill(0x1a252f);
    // Bottom-right corner
    window.rect(x + windowWidth + 2 - cornerSize, 5 + windowHeight + 2 - cornerSize, cornerSize, cornerSize);
    window.fill(0x1a252f);
...
const createBookshelf = (x, y, camera, colliders) => {
  ...
  const bookColors = [
    0xe74c3c, 0x3498db, 0x27ae60, 0xf1c40f, 0x9b59b6,
    0xe67e22, 0x1abc9c, 0x34495e, 0x8e44ad, 0xc0392b,
    0x16a085, 0x2980b9, 0x7f8c8d, 0xd35400, 0x8b4513
  ];
  ...
  window.interactables.push({
    label: "bookshelf",
    bounds,
    message: "A handcrafted wooden bookshelf filled with colorful books. The spines show signs of age and use.",
    bubble: null
  });
...
const createOfficeCat = (x, y) => {
    ...
    cat.addChild(body, tail, head, ears, innerEars, muzzle, eyes, nose, mouth, paws, whiskers);
    ...
    const animate = () => {
        ...
        if (earTwitchTimer % 200 === 0 && Math.random() < 0.8) { // Adjust frequency
...
// app.stage.addChild(officeCat);
  ...
  addWithCollider(camera, confTable, {
  x: 600 - 104,
  y: 400 - 50,
  width: 205,
  height: 105
  ...
  // Whiteboards
        const whiteboard1 = createWhiteboard(38, 250,camera,gameState.colliders);
...
const createVendingMachine = (x, y, camera, colliders) => {
  ...
  machine.addChild(body, brandText, glassFront, shelves, snacks, controlPanel, coinSlot, coinLabel, buttons, dispensingSlot, display);
  ...
  const animate = () => {
    ...
    if (Math.random() < 0.01) {
      const randomSnack = snacks.children[Math.floor(Math.random() * snacks.children.length)];
  ...
  window.interactables.push({
    label: "vending-machine",
    bounds,
    message: "The vending machine hums and glows softly. LED indicators pulse rhythmically, but the coin slot seems jammed.",
    bubble: null
  });
...
const createWaterDispenser = (x, y, camera, colliders) => {
  ...
  window.interactables.push({
    label: "elevator",
    bounds,
    message: "You watch the endless flow of pixels. This water dispenser is decorative",
    bubble: null
  });
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      switch (key) {
          ...
          camera.addChild(createElevator(offsetX + roomWidth / 2 - 30, offsetY + 70, camera, gameState.colliders));
          
          // Digital clock above elevators
          camera.addChild(createDigitalClock(offsetX-25+ roomWidth/2 - 40, offsetY , camera, gameState.colliders));
          
          // Reception counter (front and center)
          camera.addChild(createReceptionCounter(offsetX+ + roomWidth/2, offsetY + 400, camera, gameState.colliders));
          
          // Lobby seating areas (left and right sides)
          camera.addChild(createLobbySeating(offsetX + 450, offsetY + 520, camera, gameState.colliders));
          camera.addChild(createLobbySeating(offsetX + roomWidth - 200, offsetY + 520, camera, gameState.colliders));
          
          
          // Vending machines along the walls
          camera.addChild(createVendingMachine(offsetX + 490, offsetY +10,camera, gameState.colliders));
          camera.addChild(createVendingMachine(offsetX + roomWidth - 240, offsetY + 10, camera, gameState.colliders));
          
          // Water dispensers
          camera.addChild(createWaterDispenser(offsetX + 200, offsetY + 30, camera, gameState.colliders));
          camera.addChild(createWaterDispenser(offsetX + roomWidth - 60, offsetY + 30, camera, gameState.colliders));
          
          // Plants for decoration (corners and strategic spots
          camera.addChild(createPlant(offsetX + 40 , offsetY + 145, camera, gameState.colliders ));
          camera.addChild(createPlant(offsetX + 40 , offsetY + 340, camera, gameState.colliders ));
          camera.addChild(createPlant(offsetX + roomWidth - 40, offsetY + 140, camera, gameState.colliders ));
          camera.addChild(createPlant(offsetX + 600, offsetY + 400, camera, gameState.colliders ));
        ...
        case "0,1": // Open Office Area
          ...
          //Elevator
           camera.addChild(createdoor(offsetX + roomWidth/2 + 250, offsetY + 44, camera, gameState.colliders, { doorType: 'office' }));
             // Vending machines along the walls
          camera.addChild(createVendingMachine(offsetX + 490, offsetY +10,camera, gameState.colliders));
          camera.addChild(createVendingMachine(offsetX + roomWidth - 240, offsetY + 10, camera, gameState.colliders));
          ...
          // Digital clock above elevators
          camera.addChild(createDigitalClock(offsetX-25+ roomWidth/2 - 40, offsetY + 5 , camera, gameState.colliders));

          camera.addChild(createBookshelf(offsetX + 220, offsetY,camera, gameState.colliders ));


           // Lobby seating areas (left and right sides)
          camera.addChild(createLobbySeating(offsetX + 25 , offsetY + 520,camera, gameState.colliders));
          camera.addChild(createLobbySeating(offsetX + roomWidth - 260, offsetY + 520, camera, gameState.colliders));
          
          // Printer station
          camera.addChild(createTable(offsetX + 680, offsetY + 530, camera, gameState.colliders));
          ...
          // Study table
          camera.addChild(createStudyTable(offsetX + 440, offsetY + 565, camera, gameState.colliders));

          //Floor mat
          camera.addChild(createFloorMat(offsetX + 300, offsetY + 530,  Math.PI/2));

          camera.addChild(createDecorTable(offsetX + 170, offsetY + 530, camera, gameState.colliders));
        ...
        case "1,1": // Executive Office
          ...
          camera.addChild(createOfficeCounters(offsetX + 220, offsetY + 70, 2, camera, gameState.colliders));
          camera.addChild(createOfficeCounters(offsetX + 240, offsetY + 470, 7 , camera, gameState.colliders));
          camera.addChild(createCat(offsetX + 120 , offsetY + 400 , true, camera, gameState.colliders));
          camera.addChild(createPlant(offsetX + 40 , offsetY + 280, camera, gameState.colliders ));
          camera.addChild(createPlant(offsetX + 40 , offsetY + 390, camera, gameState.colliders ));
          camera.addChild(createPlant(offsetX + 540 , offsetY + 220, camera, gameState.colliders ));
          camera.addChild(createPlant(offsetX + 540 , offsetY + 390, camera, gameState.colliders ));
          camera.addChild(createFloorMat(offsetX + 60, offsetY + 210));
          camera.addChild(createVerticalLeftSofa(offsetX + 660, offsetY + 240, camera, gameState.colliders));
          camera.addChild(createSittingNPC(offsetX + 655, offsetY + 290,camera,gameState.colliders,gameState));
           camera.addChild(createCat(offsetX + 670 , offsetY + 220 ,false, camera, gameState.colliders));
...
const gameLoop = (gameState, app) => {
  ...
  if (moving) {
    ...
    if (dx !== 0 && dy !== 0) {
      ...
      if (!willCollide(testX, testY, colliders)) {
        newX = testX;
        newY = testY;
        moved = true;
      }
    ...
    if (!moved) {
      ...
      if (dx !== 0) {
        const testX = originalX + dx;
        if (!willCollide(testX, originalY, colliders)) {
          newX = testX;
          moved = true;
        }
      }
      ...
      if (dy !== 0) {
        const testY = originalY + dy;
        if (!willCollide(originalX, testY, colliders)) {
          newY = testY;
          moved = true;
        }
      }
      ...
      if (moved && newX !== originalX && newY !== originalY) {
        if (!willCollide(newX, newY, colliders)) {
...
const createTextBubble = (x, y, text, duration = 3000, parent) => {
  ...
  bubble.drawRoundedRect(0, 0, bubbleWidth, bubbleHeight * 0.6, cornerRadius);
  bubble.endFill();
  bubble.lineStyle(1.5, 0xe9ecef, 0.9);
  bubble.drawRoundedRect(0, 0, bubbleWidth, bubbleHeight, cornerRadius);
  bubble.lineStyle(0);
  bubble.beginFill(0xffffff, 0.6);
  bubble.drawRoundedRect(1, 1, bubbleWidth - 2, bubbleHeight * 0.4, cornerRadius - 1);
  bubble.endFill();
  ...
  const exitAnimation = () => {
    ...
    const step = () => {
      ...
      if (progress < 1) {
        ...
        const interactable = (window.interactables || []).find(obj => obj.bubble === container);
        if (interactable) {
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: "12px", opacity: 0.8 }}>
              Use WASD to move • Press Enter to chat
            </div>
          </div>
